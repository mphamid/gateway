...
class GatewayServiceProviderLaravel5 extends ServiceProvider
{
	...
	public function boot()
	{
        ...
        //php artisan vendor:publish --provider=mphamid\Gateway\GatewayServiceProvider --tag=config
        $this->publishes([
            $config => config_path('gateway.php'),
        ], 'config');

        // php artisan vendor:publish --provider=mphamid\Gateway\GatewayServiceProvider --tag=migrations
        $this->publishes([
        ...
        // php artisan vendor:publish --provider=mphamid\Gateway\GatewayServiceProvider --tag=views
        $this->publishes([
